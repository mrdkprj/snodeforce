...
class GridTable {
	...
	constructor(rootElement, params) {
		...
		this.calculator.classList.add("gtbl-value-cell");
		this.rootNode.appendChild(this.calculator)
		this.sizeBase = Util.getSizeBase(this.header, this.rows, this.rootElement.css("font"), this.calculator, this.baseRowHeight,this.rootNode);
		...
		this.rootElement.css({overflow:"auto", position:"relative"});
	...
	createGridTable(){
		...
		const getContainer = () => {
			return $("<div>", { class: "gtbl-container", css: {"height": this.totalContentHeight + "px"}});
		}
		...
		*/
		...
		this.editor.on("keyup", this.onEditorKeyup.bind(this));
		this.editor.on("paste", this.onEditorPaste.bind(this))
		this.inputHolder = $("<div>", {class:"input-holder"});
		this.inputHolder.append(this.editor);
		this.rootElement.append(this.inputHolder);

		this.cloneHeader = $("<div>", {class:"clone-header", css:{width: parseInt(this.columnWidths[0]) + "px"}});
		this.cloneHeader.on("mousedown", this.onCloneHeaderClick.bind(this));
	...
	createRow(rowIndex){
		...
		const rowHeaderCell = $("<div>", { class: "gtbl-header-cell gtbl-row-header-cell", css:{"transform": "translateX(0px)"}, text: rowIndex + 1 });
	...
	onSlidebarMousedown(e){
		...
		this.init = e.pageX;
		this.resizingCell = $(e.target).parent();
		this.resizingCell.addClass("noselect");
		this.rootElement.css("cursor", "col-resize");
		this.firstCell = this.visibleNodes[0].children().eq(this.resizingCell.index());
	}

	resizeColumn(e){
		const by = e.pageX - this.init;
		this.init = e.pageX;
		const containerWidth = parseInt(this.resizingCell.css("width").replace("px", ""));
		const newWidth = containerWidth + by;
		if(newWidth > 2){
			this.resizingCell.css("width", newWidth + "px");
			this.firstCell.css("width", newWidth + "px");
		}
		return false;
	}
	...
	resizeRow(e){
		const by = e.pageY - this.init;
		this.init = e.pageY;
		const containerWidth = parseInt(this.resizingCell.css("height").replace("px", ""));
		const newWidth = containerWidth + by;
		if(newWidth > 2){
			this.resizingCell.css("height", newWidth + "px");
		}
		return false;
	}
	...
	onDocumentMouseUp(e) {
		this.isDragging = false;
		if(this.isResizing){
			this.resizingCell.removeClass("noselect");
			this.sizeBase.heights[this.resizingCell.parent().index()] = this.resizingCell.outerHeight(true);
			this.prepareVirtualScroll(this.rootElement.scrollTop(), this.rootElement.scrollLeft(), true)
			this.container.css("height", this.totalContentHeight);
			this.alterTransform();
		}
	...
	recalculateSize(value){
		...
		let height;
		if(len > this.sizeBase.heights[this.current.Cell.RowIndex]){
			height = this.baseRowHeight * len - (len - 1);
			this.sizeBase.heights[this.current.Cell.RowIndex] = len;
		}else{
	...
	onEditorKeydown(e){
		if(e.key == "Enter"){
			if(e.altKey || e.ctrlKey){
				...
				const size = this.recalculateSize(this.editor.val());
				this.editor.height(size.height);
			}else{
				this.endEdit();
	...
	onEditorKeypress(e){
		const size = this.recalculateSize(this.editor.val() + e.key);
		this.editor.width(size.width);
		this.editor.height(size.height);
	}
	...
	onEditorPaste(e){
		...
		const str = currentText.slice(0, caretPosition) + pastedData + currentText.slice(caretPosition)
		this.editor.val(str);
	...
	prepareEditor(value){
		...
		const headerPos = this.rowHeaderCells[this.current.Node.parent().index()].position();
...
class EditHistory{
	...
	begin(cellNode, value, height){
		this._bef = {undo:{CellNode:cellNode, value:value, height: height}};
	}

	end(value, height){
		if(this._bef.value == value){
			this._bef = null;
			return;
		}

		this._redoStack = [];
		this._bef.redo = {CellNode:this._bef.undo.CellNode, value:value, height: height}
		this._undoStack.push(this._bef);
		this.rows[this._bef.undo.CellNode.Cell.RowIndex][this._bef.undo.CellNode.Cell.ColumnIndex] = value;
	}
...
class Util{
	...
	static reduceRowHeights(a, b){
		...
		const len = this.css(this.baseElement, "height");

		return len > a ? len : a;
	}

	static reduceRowHeights2(a, b){
