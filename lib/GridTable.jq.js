...
class GridTable {
	...
	createGridTable(){
		...
		this.editor.blur(this.endEdit.bind(this));
		...
		this.cloneHeader.on("click", this.onCloneHeaderClick.bind(this));
	...
	onCloneHeaderClick(e){
		this.initEditor();
		this.selectRow(this.rowHeaderCells[this.current.Node.parent().index()])
	}
	...
	prepareEditor(){
		const position = this.current.Node.position();
		this.inputHolder.css({
			position:"absolute",
			top: position.top + 1 + this.headerHeight,
		...
		this.cloneHeader.css({
			position:"absolute",
			top: headerPos.top + this.headerHeight,
			left: headerPos.left,
			height: this.current.Node.height() + 1,
		});
		this.cloneHeader.html(this.current.Node.parent().index() + 1)

		this.editor.css({
			width: this.current.Node.outerWidth(true) - 2,
			height: this.current.Node.height() - 1,
		})
