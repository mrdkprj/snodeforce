...
class GridTable {
	...
	createGridTable(){
		...
		*/
		...
		this.cloneHeader = $("<div>", {class:"clone-header", css:{width: this.columnWidths[0] + "px"}});
	...
	onSlidebarMousedown(e){
		...
		this.init = e.pageX;
		this.resizingColumn = $(e.target).parent();
		this.resizingColumn.addClass("noselect");
		this.rootElement.css("cursor", "col-resize");
		this.firstCell = this.visibleNodes[0].children().eq(this.resizingColumn.index());
	}
	...
	moveSlidebar(e){
		...
		this.resizeAnimationFrame = window.requestAnimationFrame(() => this.resizeColumn(e));
	}

	resizeColumn(e){
		const by = e.pageX - this.init;
		this.init = e.pageX;
		const containerWidth = parseInt(this.resizingColumn.css("width").replace("px", ""));
		const newWidth = containerWidth + by;
		if(newWidth > 2){
			this.resizingColumn.css("width", newWidth + "px");
	...
	onDocumentMouseUp(e) {
		this.isDragging = false;
		if(this.isResizing){
			this.resizingColumn.removeClass("noselect");
		}
		this.isResizing = false;
	...
	recalculateSize(value){
		...
		const data = (this.editor.val().trim() + value).split("\n");
	...
	onEditorKeydown(e){
		if(e.key == "Enter"){
			if(e.altKey || e.ctrlKey){
				...
				const size = this.recalculateSize("\n");
	...
	onEditorKeypress(e){
		const size = this.recalculateSize(e.key);
	...
	beginEdit(value){
		...
		this.history.begin(this.current, this.current.Node.html());
		this.editor.focus();
	}
	...
	endEdit(){
		...
		//const height =
		this.history.end(this.editor.val());
		this.initEditor();
	...
	undoEdit(){
		...
		this.selectByMouseDown(memento.Node)
	}
...
class EditHistory{
	...
	begin(cellNode, value){
		this._bef = {CellNode:cellNode, prev:value, next:null};
	}

	end(value){
		if(this._bef.prev == value){
			return;
		}

		this._redoStack = [];
		this._bef.next = value;
		this._undoStack.push(this._bef);
		this.rows[this._bef.CellNode.Cell.RowIndex][this._bef.CellNode.Cell.ColumnIndex] = value;
	}
	...
	undo(){
		...
		this.rows[undo.CellNode.Cell.RowIndex][undo.CellNode.Cell.ColumnIndex] = undo.prev;
		undo.CellNode.Node.html(undo.prev);
		return undo.CellNode;
	}
	...
	redo(){
		...
		this.rows[redo.CellNode.Cell.RowIndex][redo.CellNode.Cell.ColumnIndex] = redo.next;
		redo.CellNode.Node.html(redo.next);
		return redo.CellNode;
	}
