...
class GridTable {
	...
	createGridTable(){
		...
		const getFilterDialog = () => {
			...
			filterDialog.style.width = "0px";
			filterDialog.style.height = "0px";
			filterDialog.style.position = "absolute";
			filterDialog.style.visibility = "hidden";
			filterDialog.style.top = "0px";
			filterDialog.style.left = "0px";
			filterDialog.style.background = "white";
			filterDialog.style.border = "1px solid #ccc";
			return filterDialog;
		}
		...
		this.dropdown = getFilterDialog();
		this.viewport.appendChild(this.dropdown);
	}
	...
	onFilterColumnClick(e){
		...
		this.dropdown.style.top = e.target.offsetTop +28+ "px";
		this.dropdown.style.left = e.target.offsetLeft + "px";
		const idx = this.indexOf(e.target.parentNode)
		const ul = document.createElement("ul")
		const lis = document.createDocumentFragment();
		this.columnValues[idx].forEach(e => {
	...
	filter(columnIndex, value){
		...
		if(Array.isArray(value)){
			this._filterByValues(columnIndex, values);
		}else{
...
class Util{
	...
	static getSizeBase(header, rows, styles){
		...
		const baseData = this.transpose([header].concat(rows));
		const _maxLengthValues = baseData.map(item => item.reduce(this.compareLength.bind(this)));
