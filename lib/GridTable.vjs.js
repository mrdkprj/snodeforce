...
class GridTable {
	...
	createGridTable(){
		...
		const getFocusHolder = () => {
			...
			focusHolder.setAttribute("value","");
	...
	scrollHorizontally(target, padding){
		...
		if(this.rootNode.scrollWidth == this.css(target, "width") + position.left){
			this.alterScrollPosition(null, this.rootNode.scrollWidth);
			return;
		}

		if(position.left - this.css(target.previousElementSibling, "width") == 0){
			this.alterScrollPosition(null, 0);
			return;
		}
		...
		if(scrollLeft + this.css(this.rootNode, "width") <= position.left + this.css(target, "width") + this.getScrollbarHeight()){
			const scrollby = ((position.left + this.css(target, "width")) + this.getScrollbarHeight()) - (scrollLeft + this.css(this.rootNode, "width"));
			if(padding){
