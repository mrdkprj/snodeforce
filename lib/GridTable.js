...
const GridTable = (() => {
	...
	return class GridTable {
		...
		constructor(rootElement, data) {
			...
			privates(this).scrollInterval = null;
		...
		moveCellByCtrlArrowKey(direction, keepCurrentCell){

			this.bypassHighlightByScroll = !keepCurrentCell//true;
			this.preferredDirection = direction;
			this.keepCurrentCell = keepCurrentCell;
			if(keepCurrentCell){
		...
		moveCellByArrowKey(direction, keepCurrentCell){

			this.bypassHighlightByScroll = !keepCurrentCell;
			this.preferredDirection = direction;
			this.keepCurrentCell = keepCurrentCell;
			if(keepCurrentCell){
				this.anchor = this.last;
		...
		scrollTo(cellNode){
			...
			if(this.rootElement.scrollLeft() > cellNode.Node.position().left){
				this.alterScrollPosition(null, cellNode.Node.position().left - this.columnWidths[0]);
				scrollRequired = true;
			}

			if(this.rootElement.outerWidth(true) + this.rootElement.scrollLeft() < cellNode.Node.position().left){
				position = cellNode.Node.position().left - (this.rootElement.outerWidth(true) + this.rootElement.scrollLeft());
				this.alterScrollPosition(null, cellNode.Node.position().left - position + this.barWidth);
				scrollRequired = true;
			}
		...
		scrollHorizontally(target, padding){
			...
			if(scrollLeft >= position.left){
				...
				this.alterScrollPosition(null, position.left - this.columnWidths[0] - pad);
				return;
			}
			...
			if(scrollLeft + this.rootElement.outerWidth(true) <= position.left + target.outerWidth(true) + this.barHeight){
				...
				this.alterScrollPosition(null, scrollLeft + by + pad);
				return;
			}
		...
		scrollVertically(target, padding){
			...
			if(scrollTop >= position.top){
				...
				this.alterScrollPosition(position.top - pad);
				return;
			}
			...
			if(scrollTop + this.visibleViewportHeight <= position.top + target.outerHeight(true) + this.barHeight){
				...
				this.alterScrollPosition(scrollTop + by + pad);
				return;
			}
		...
		onDocumentMouseMove(e){
			...
			const currentDelta = getDelta(e.originalEvent);

/*
			if(this.lastDelta == currentDelta){
				return true;
			}else{
...
*/
			...
			privates(this).scrollInterval = window.setInterval(this.keepScroll.bind(this), 50, currentDelta);
		}
		...
		keepScroll(direction){
			...
			const scrollToNextCell = (direction) => {
				...
				return this.scrollTo(nextCellNode);
			}
		...
		resetInterval(){
			if(privates(this).scrollInterval){
				window.clearInterval(privates(this).scrollInterval);
				privates(this).scrollInterval = null;
			}
		}
...
/*
			if(e.shiftKey){
				switch(e.keyCode){
					// Ctrl + End
					case 35:
						this.moveCellByCtrlArrowKey(this.Delta.End);
						return false;
					// Ctrl + Home
					case 36:
						this.moveCellByCtrlArrowKey(this.Delta.Home);
						return false;
					// Ctrl + Left
					case 37:
						this.moveCellByCtrlArrowKey(this.Delta.Left);
						return false;
					// Ctrl + Right
					case 39:
						this.moveCellByCtrlArrowKey(this.Delta.Right);
						return false;
					// Ctrl + Up
					case 38:
						this.moveCellByCtrlArrowKey(this.Delta.Up);
						return false;
					// Ctrl + Down
					case 40:
						this.moveCellByCtrlArrowKey(this.Delta.Down);
						return false;
				}
			}
*/
