...
const GridTable = (() => {
	...
	return class GridTable {
		...
		prepareVirtualScroll(scrollTop, scrollLeft, reset){
			...
			function getChildHeight(index){
				if(_this.sizeBase.heights[index] > 1)
					return (_this.rowHeight * _this.sizeBase.heights[index]) + index;
				else
			...
			this.startNode = Math.max(0, firstVisibleNode - this.renderAhead);

			const lastVisibleNode = findEndNode(this.childPositions, firstVisibleNode, this.itemCount, this.rootHeight);
			const endNode = Math.min(this.itemCount - 1, lastVisibleNode + this.renderAhead);
			//this.renderAhead = endNode;
			...
			//}
			console.log( this.visibleNodesCount)
		...
		prepareVirtualScroll2(scrollTop, scrollLeft, reset){
		...
		doVirtualScroll(e){
			...
			if(this.visibleNodesCount < this.visibleNodes.length){
				console.log("aaaaa")
				const sss = this.visibleNodesCount;
				const end = (this.visibleNodes.length - this.visibleNodesCount) + this.visibleNodesCount;
				console.log(sss)
				console.log(end)
				for(let i = sss; i < end; i++){
					_this.visibleNodes[i].remove();
				}
			}
		...
		onCellMouseOver(e) {
			...
			const isSelectingX = this.scrollX(cell);
			const isSelectingY = this.scrollY(cell, 10);
			this.scrolledBySelection = isSelectingX || isSelectingY;
		}
		...
		scrollY(target, padding){
			...
			if(scrollTop + this.visibleViewportHeight <= position.top + pad){
				//this.rootElement.scrollTop(scrollTop + this.rowHeight);
