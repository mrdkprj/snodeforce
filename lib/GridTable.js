...
class GridTable {
	...
	createGridTable(){
		...
		const getSearchDialog = () => {
			...
			dialog.style.top = (this.rootNode.getBoundingClientRect().top + 0) + "px";
			dialog.style.left = (this.rootNode.getBoundingClientRect().right - 280) + "px";
	...
	changeRowValue(rowArray, arrayIndex, keepCurrent){
		...
		rowArray.forEach((value, index) => {
			...
			if(index == 0){
				...
				if(this.searchUtil.has(rowIndex, index)){
	...
	scrollRequired(cellNode){
		...
		// hidden right
		if(Util.css(this.rootNode, "width") + scrollLet < positionLeft){
			const position = positionLeft - (Util.css(this.rootNode, "width") + scrollLet);
			const barWidth = Util.css(this.rootNode, "width") - this.rootNode.clientWidth;
	...
	scrollHorizontally(target, padding){
		...
		if(this.rootNode.scrollWidth == Util.css(target, "width") + positionLeft){
			this.alterScrollPosition(null, this.rootNode.scrollWidth);
			return;
		}
		...
		if(scrollLeft + Util.css(this.rootNode, "width") <= positionLeft + Util.css(target, "width") + this.getScrollbarHeight()){
			const scrollby = ((positionLeft + Util.css(target, "width")) + this.getScrollbarHeight()) - (scrollLeft + Util.css(this.rootNode, "width"));
			if(padding){
	...
	scrollVertically(target, padding){
		...
		if(this.rootNode.scrollHeight == Util.css(target, "height") + positionTop){
			this.alterScrollPosition(this.rootNode.scrollHeight);
			return;
		}
	...
	onDocumentMouseMove(e){
		...
		const getDirection = (e) => {
			...
			if(e.clientX > Util.css(this.rootNode, "width")){
	...
	calculateEditorSize(value){
		...
		let width = Util.getStringWidth(maxLengthValue, false, Util.css(this.rootNode, "font")) - 28;
	...
	closeSearchDialog(){
		this.viewport.querySelectorAll(".found").forEach(el => el.classList.remove("found"));
...
class SearchUtil{
	...
	startSearch(rows, value, currentRowIndex){
		...
		const closestIndex = Util.findClosest(currentRowIndex, this.Results, this.Results.length, "RowIndex");
	...
	has(rowIndex, columnIndex){
		...
		return this._mp.hasOwnProperty(rowIndex) && this._mp[rowIndex].includes(columnIndex);
	}
}
