	...
	onFocusHolderKeydown(e){
		...
		if(e.key == "F3"){
			if(this.dialog.style.display == "block"){
				...
				e.preventDefault();
				if(e.shiftKey){
					this.doSearch(this.Direction.Up);
				}else{
					this.doSearch(this.Direction.Down);
				}
				return;
			}
		}
...
class SearchUtil{
	...
	continueSearch(direction){
		...
		const nextIndex = direction == this.Direction.Down ? this.CurrentResult.Index + 1 : this.CurrentResult.Index - 1;

		if(direction == this.Direction.Down && this.Results.length == nextIndex){
			return false;
		}

		if(direction == this.Direction.Up && nextIndex == -1){
			return false;
		}
