...
const GridTable = (() => {
	...
	return class GridTable {
		...
		constructor(rootElement, data) {
			...
			this.barHeight = this.rootHeight - rootElement.clientHeight;
		}
		...
		assignEventHandlers(){
			...
			this.focusHolder.on("keydown", this.onFocusHolderKeyDown.bind(this));
		}
		...
		prepareVirtualScroll(scrollTop, scrollLeft, reset){
			...
			function getChildHeight(index){
				if(_this.sizeBase.heights[index] > 1){
					return (_this.rowHeight * _this.sizeBase.heights[index]) - (_this.sizeBase.heights[index] - 1);
				}else{
					return _this.rowHeight;
				}
			}

			function getChildPositions(itemCount){
				const results = [0];
			...
			if(reset){
				...
				this.totalContentHeight = this.childPositions[this.itemCount - 1] + getChildHeight(this.itemCount - 1) + this.nodePadding;
			}

			const firstVisibleNode = findStartNode(scrollTop, this.childPositions, this.itemCount);

			this.startNode = Math.max(0, firstVisibleNode - this.nodePadding);

			const lastVisibleNode = findEndNode(this.childPositions, firstVisibleNode, this.itemCount, this.rootHeight);
			const endNode = Math.min(this.itemCount - 1, lastVisibleNode + this.nodePadding);

			this.visibleNodesCount = endNode - this.startNode + this.nodePadding;
		...
		prepareVirtualScroll_old(scrollTop, scrollLeft, reset){
		...
		createHeader(){
			...
			_cornerCell.css("min-width", this.columnWidths[0] + "px");
			...
			this.header.forEach(function (item, index) {
				...
				header.css("min-width", _this.columnWidths[index + 1] + "px");
		...
		createRow(rowIndex){
			...
			if(_isFirstRow){
				...
				_rowHeaderCell.css("min-width", this.columnWidths[0] + "px");
			}
			...
			_row.forEach(function (cellvalue, cellIndex) {
				...
				if(_isFirstRow){
					...
					cell.css("min-width",_this.columnWidths[cellIndex + 1] + "px");
				}
		...
		getVisibleChildNodes(){
			...
			return new Array(this.visibleNodesCount)
				.fill(null)
				.map((_, index) => _this.createRow(index + _this.startNode));
		}
		...
		doVirtualScroll(e){
			...
			const changeRowValue = function(rowArray, arrayIndex){
				...
				if(arrayIndex > _this.visibleNodes.length - 1){
					const newItem = _this.createRow(arrayIndex);
					_this.visibleNodes.push(newItem);
					_this.viewport.append(newItem);
				}
				...
				rowArray.forEach((value, index) => {
					...
					if(_this.current && _this.current.Cell.RowIndex == rowIndex && _this.current.Cell.ColumnIndex == index - 1){
						_this.markCurrent($(node));
					}

				});
			}

			if(!this.preventVirtualScroll){
				...
				if(this.visibleNodesCount < this.visibleNodes.length - 1){
					const end = this.visibleNodes.length - 1;
					const count = end - this.visibleNodesCount
					_this.visibleNodes.splice(this.visibleNodesCount, count).forEach(el => el[0].remove());
				}
			}else{
				this.preventVirtualScroll = false;
			}
		...
		markCurrent(cell){
			...
			if(this.current){
				this.current.Node.removeClass("current");
				this.currentRowHeaderCell.removeClass("current-row");
			}

			this.current = this.getCellNode(cell);
			this.currentRowHeaderCell = cell.parent().children().eq(0);

			this.current.Node.addClass("current");
			this.currentRowHeaderCell.addClass("current-row");
			...
			this.isMarkScroll = this.scrolledBySelection;
		}
		...
		onCellMouseOver(e) {
			...
			const isSelectingX = this.scrollX(cell);
			const isSelectingY = this.scrollY(cell, this.rowHeight);
			if(isSelectingX){
				this.preventVirtualScroll = true;
			}
			this.scrolledBySelection = isSelectingX || isSelectingY;
		}
		...
		scrollX(target){
			...
			console.log("roor:"+this.rootElement.width())
			console.log("pos:"+position.left)
			console.log("target:"+target.outerWidth(true))
			*/
			if(scrollLeft >= position.left){
				this.rootElement.scrollLeft(position.left -  this.columnWidths[0]);
				return true;
			}

			if(scrollLeft + this.rootElement.width() <= position.left + target.outerWidth(true) + this.barHeight){
				const by = ((position.left + target.outerWidth(true)) + this.barHeight) - (scrollLeft + this.rootElement.width());
				this.rootElement.scrollLeft(scrollLeft + by);
				return true;
			}
		...
		scrollY(target, padding){
			...
			if(scrollTop + this.visibleViewportHeight <= position.top + pad + target.outerHeight(true) + this.barHeight){
