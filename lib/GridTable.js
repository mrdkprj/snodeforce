...
const GridTable = (() => {
	...
	return class GridTable {
		...
		moveCurrentCellByCtrlArrowKey(direction){
			...
			this.bypassHighlightByScroll = true;
		...
		moveCellByArrowKey(direction, keepCurrentCell){
			...
			if(this.scrollTo(this.anchor)){
				this.scrollCallback = this.changeByArrowKey;
			}else{
				this.changeByArrowKey();
			}
		}
		...
		scrollTo(cellNode){
			...
			if(cellNode.Cell.RowIndex > this.startNode + this.visibleNodesCount){
				const position = this.baseRowHeight * (cellNode.Cell.RowIndex - (this.visibleNodesCount - 3));
			...
			if(cellNode.Cell.RowIndex < this.startNode){
				const position = this.baseRowHeight * cellNode.Cell.RowIndex;
