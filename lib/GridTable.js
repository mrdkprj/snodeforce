	...
	return class GridTable {
		...
		constructor(rootElement, data) {
			...
			*/
			...
			this.renderAhead = this.rowHeight;
		...
		prepareVirtualScroll(scrollTop, scrollLeft, reset){
			...
			if(reset){
				...
				this.totalContentHeight = this.childPositions[this.itemCount - 1] + getChildHeight(this.itemCount - 1);// + 35;
			}
			...
			//this.visibleViewportHeight = this.childPositions[lastVisibleNode - 2] - this.childPositions[firstVisibleNode];
		...
		createTable(){
			...
			this.baseVisibleNodesCount = this.visibleNodes.length;
		...
		doVirtualScroll(e){
			...
			const changeRowValue = function(rowArray, arrayIndex){
				...
				if(arrayIndex > _this.visibleNodes.length - 1){
					const newItem = _this.renderItem(arrayIndex);
		...
		scrollX(target){
			...
			*/
			...
			if(scrollLeft + this.rootElement.width() <= position.left + target.outerWidth(true) + this.barHeight){
				...
				var by = ((position.left + target.outerWidth(true)) + this.barHeight) - (scrollLeft + this.rootElement.width());
		...
		scrollY(target, padding){
			...
			if(scrollTop + this.visibleViewportHeight <= position.top + pad + target.outerHeight(true) + this.barHeight){
				var by = (position.top + target.outerHeight(true)) - (scrollTop + this.visibleViewportHeight) + this.barHeight;
