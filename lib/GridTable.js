...
const GridTable = (() => {
	...
	return class GridTable {
		...
		changeCurrentByArrowKey(){
			...
			switch(this.preferredDirection){
				case this.Delta.Home:
					cell = this.current.Node.parent().children().eq(1);
					break;
				case this.Delta.End:
					cell = this.current.Node.parent().children().eq(this.header.length);
					break;
				case this.Delta.Left:
		...
		changeHighlightByScroll(){
			...
			const changeHighlightRequired = () => {
				...
				if(this.isAllSelected || this.isColumnSelected){
					return true;
				}
...
*/
			...
			privates(this).scrollInterval = window.setInterval(this.keepScroll, 50, currentDelta);
		}
		...
		triggerCellMouseOver(){
			if(!this.nextCell){
				if(this.preferredDirection == this.Delta.Up){
...
*/
			switch (e.keyCode) {
				// Ctrl + End
				case 35:
					this.moveCurrentCellByArrowKey(this.Delta.End);
					return false;
				// Ctrl + Home
				case 36:
					this.moveCurrentCellByArrowKey(this.Delta.Home);
					return false;
				// Left
				case 37:
...
class Util{
	...
	static compareLength(a, b){
		const left = a ? a.toString().split("\n").reduce(function (a, b) { return a.length > b.length ? a : b; }) : "";
		const right = b ? b.toString().split("\n").reduce(function (a, b) { return a.length > b.length ? a : b; }) : "";
