...
class GridTable {
	...
	constructor(rootElement, data) {
		...
		this.columnWidths = calculateWidths(this.header, this.rows, this.rootElement.css("font"));
	...
	createGridTable(){
		...
		const _rootScrollTop = this.rootElement.scrollTop();

		this.startNode = Math.floor(_rootScrollTop / this.rowHeight) - this.nodePadding;
	...
	resetSelectionDataRange(){
		this.setSelectionDataRange(0,0,0,0);
	}
...
const calculateWidths = (header, rows, font) => {
	...
	function reduceString(array){
		return transpose(array).map(item => item.reduce(compareLength));
	}
	...
	const _font = font;
	const _numberColumnWidth = getStringWidth(rows.length, false);
	const _maxLengthValues = reduceString([header].concat(rows));

	return [_numberColumnWidth].concat(_maxLengthValues.map(item => getStringWidth(item, true)));
}
