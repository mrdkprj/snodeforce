	...
	return class GridTable {
		...
		doVirtualScroll(e){
			...
			const getRowDataAt = (index) => {
				return [index + 1].concat(_this.rows[index]);
			}

			const addRow = (index) => {
				const newItem = _this.createRow(index);
			...
			.map((_, index) => getRowDataAt(index + _this.startNode))
			...
			if(this.visibleNodesCount < this.visibleNodes.length - 1){
				...
				_this.rowHeaderCells.splice(this.visibleNodesCount, count);
			}
		...
		changeHighlight(cell) {
			...
			for (var i = rowStart; i <= rowEnd; i++) {
				...
				_this.rowHeaderCells[i].addClass("row-highlight");
		...
		changeHighlightByScroll(){
			...
			if(this.last.Cell.RowIndex)
			if(!this.updateVirtualSelection(this.last)){
				return true;
			}
		...
		updateVirtualSelection(target){
			...
			if(this.current.Cell.RowIndex >= target.Cell.RowIndex){
				...
				if(target.Cell.RowIndex > this.startNode + this.visibleNodesCount){
					return false;
				}
				/*
				console.log("--------------")
		...
		outOfRange(){
			...
			const mx = Math.max(this.current.Cell.RowIndex, this.last.Cell.RowIndex)-1;
			const mn = Math.min(this.current.Cell.RowIndex, this.last.Cell.RowIndex)-1;

			if(mn < this.startNode && mx < this.startNode){
				return true;
			}else{
				return false;
			}
