...
class GridTable {
	...
	changeRowValue(rowArray, arrayIndex, keepCurrent){
		...
		const shouldMarkAsCurrent = (rowIndex, colIndex) => {
									//test
									if(this._nextSearchResult.RowIndex == rowIndex && this._nextSearchResult.ColumnIndex == colIndex - 1){
										return true;
									}
									//test
	...
	alterTransform(){
		this.viewport.style.top = this.nodeOffsetY + "px";
	}
	...
	scrollRequired(cellNode){
		...
		const positionTop = this.childPositions[cellNode.Cell.RowIndex];
		const positionLeft = (this.rootNode.getBoundingClientRect().left - scrollLet) + this.columnPositions[cellNode.Cell.ColumnIndex + 1];
	...
	search(direction){
		...
		const initSearch = (value) => {
			...
			this.rows.forEach( (row, rowIndex) => {
				...
				row.forEach( (col, colIndex) => {
					...
					if(Util.toStringNullSafe(col).includes(value)){

						const result = {Index: this._searchResults.length, RowIndex: rowIndex, ColumnIndex: colIndex};
		...
		if(this._searchValue == value && this._searchResults.length > 0){

			const nextIndex = direction == this.Direction.Down ? this._nextSearchResult.Index + 1 : this._nextSearchResult.Index - 1;
			if(direction == this.Direction.Down && this._searchResults.length == nextIndex){
				return;
			}else if(direction == this.Direction.Up && nextIndex == -1){
				return;
			}

			this._nextSearchResult = this._searchResults[nextIndex];

		}else{
			if(!initSearch(this._searchValue)){
				return;
			}

			const findClosest = (scrollTop, nodePositions, itemCount) => {
				let startRange = 0;
				let endRange = itemCount > 0 ? itemCount - 1 : 0;
				...
				while (endRange !== startRange) {
					...
					if (nodePositions[middle].RowIndex <= scrollTop && nodePositions[middle + 1].RowIndex > scrollTop) {
						return middle;
					}
					...
					if (nodePositions[middle].RowIndex <= scrollTop) {
						startRange = middle;
					}else{
						endRange = middle;
					}
			...
			const tar = findClosest(this.current.Cell.RowIndex, this._searchResults, this._searchResults.length);
			this._nextSearchResult = this._searchResults[tar];
		}

		const target = 	{
			Node: null,
			Cell: new Cell(this._nextSearchResult.RowIndex, this._nextSearchResult.ColumnIndex)
		};


		if(this.scrollRequired(target) == false){
			this.selectByMouseDown(this.visibleNodes[this._nextSearchResult.RowIndex - this.startNode].childNodes[this._nextSearchResult.ColumnIndex + 1])
		}
	}
	...
	onDocumentMouseUp(e) {
		...
		if(this.currentResizeMode == this.ResizeMode.Row){
			...
			this.rowHeaderCellsVirtual[this.rowIndexOf(this.resizingCell.Node)].style.height = this.css(this.resizingCell.Node, "height")  + "px";
			this.prepareVirtualScroll(this.rootNode.scrollTop, this.rootNode.scrollLeft, true);
			this.container.style.height = this.totalContentHeight + "px";
			this.alterTransform();
		}else if(this.currentResizeMode == this.ResizeMode.Column){
			this.sizeBase.widths[this.resizingCell.Cell.ColumnIndex + 1] = this.css(this.resizingCell.Node, "width");
...
/*
		const headerPositionLeft = (this.rowHeaderCellsVirtual[this.indexOf(this.current.Node.parentNode)].getBoundingClientRect().left - this.rootNode.getBoundingClientRect().left) + scrollLeft;
		...
		this.cloneHeader.childNodes[0].innerHTML = this.current.Cell.RowIndex + 1;
*/
	...
	onFocusHolderKeydown(e){
		...
		if(e.key == "F3"){
			if(this.dialog){
				...
				e.preventDefault();
				if(e.shiftKey){
					this.search(this.Direction.Up);
				}else{
					this.search(this.Direction.Down);
				}
				return;
			}
		}
	...
	onCellDblClick(e){
		if(this.readOnly || this.readOnlyColumns.has(this.indexOf(e.target) - 1)){
