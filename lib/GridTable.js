	...
	return class GridTable {
		...
		prepareVirtualScroll(scrollTop, scrollLeft, reset){
			...
			const getChildPositions = (itemCount) => {
				...
				for (let i = 1; i < itemCount; i++) {
				  results.push(results[i - 1] + _this.getChildHeight(i - 1));
				}
				return results;
			}
			const renderAhead = 1;
		...
		getChildHeight(index){
			if(this.sizeBase.heights[index] > 1){
				return (this.rowHeight * this.sizeBase.heights[index]) - (this.sizeBase.heights[index] - 1);
			}else{
		...
		createGridTable(){
			...
			const getContainer = () => {
				const container = $("<div>", { class: "gtbl-container", css: {"height": _this.totalContentHeight + "px", "overflow": "hidden", "display":"inline-block"}});
				container.id = "gtbl_".concat(_this.generateUuid());;
				return container;
			}

			const getColumnHeader = () => {

				const _columnHeader = $("<div>", { class: "gtbl-row gtbl-row-header gtbl-hidden-row-header"});

				const _cornerCell = $("<div>", { class: "gtbl-header-cell gtbl-corner-cell stick"});
				_cornerCell.width(_this.columnWidths[0] + "px");
				_cornerCell.css("min-width", _this.columnWidths[0] + "px");
				_cornerCell.on("click", _this.onCornerCellClick.bind(_this));
				_columnHeader.append(_cornerCell);

				_this.header.forEach((item, index) => {
			...
			const getFocusHolder = () => {
				const _focusHolder = $("<input class='focusHolder' type='text' value='' style='position:fixed;top:-100px;left:-100px;'/>");
				_focusHolder.on("keydown", _this.onFocusHolderKeyDown.bind(_this));
				return _focusHolder;
			}
		...
		onCellMouseOver(e) {
			...
			//const isSelectingX = this.scrollX(cell);
			const isSelectingX = this.scrollX(cell, true);
			//const isSelectingY = this.scrollY(cell, this.rowHeight);
			const isSelectingY = this.scrollY(cell, true);
			if(isSelectingX){
		...
		scrollX(target, padding){
			...
			*/
			if(scrollLeft >= position.left){
				...
				this.rootElement.scrollLeft(position.left - pad);
				return true;
			}
