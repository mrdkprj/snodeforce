...
class GridTable {
	...
	changeCellByCtrlArrowKey(){
		...
		switch(this.preferredDirection){
			...
			case this.Direction.Home:
				...
				break;
			case this.Direction.Left:
				row = this.visibleNodes[this.anchor.Node.parent().index()].get(0);
				cell = row.children[1];
				break;
			case this.Direction.Right:
				row = this.visibleNodes[this.anchor.Node.parent().index()].get(0);
			...
			case this.Direction.Up:
				row = this.visibleNodes[0].get(0);
				cell = row.children[this.anchor.Node.index()];
				break;
			case this.Direction.Down:
				row = this.visibleNodes[this.visibleNodes.length - 2].get(0);
				cell = row.children[this.anchor.Node.index()];
	...
	scrollHorizontally(target, padding){
		...
		if(scrollLeft + this.rootElement.outerWidth(true) <= position.left + target.outerWidth(true) + this.getScrollbarHeight()){
			const by = ((position.left + target.outerWidth(true)) + this.getScrollbarHeight()) - (scrollLeft + this.rootElement.outerWidth(true));
			if(padding){
	...
	scrollVertically(target, padding){
		...
		if(scrollTop + this.visibleViewportHeight <= positionTop + this.getChildHeight(targetCellNode.Cell.RowIndex) + this.getScrollbarHeight()){
			...
			const by = (positionTop + this.getChildHeight(targetCellNode.Cell.RowIndex)) - (scrollTop + this.visibleViewportHeight) + this.getScrollbarHeight();
